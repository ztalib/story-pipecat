...
class StoryCaptureHandler(BaseHandler):
    ...
    def get_schema() -> FunctionSchema:
        return FunctionSchema(
            ...
            description=(
                "Extract and save important, related story elements like people, places, "
                "events, and emotions."
            ),
            properties={
                "people": {
...
class StoryFlowHandler(BaseHandler):
    ...
    def get_schema() -> FunctionSchema:
        return FunctionSchema(
            ...
            properties={
                "story_phase": {
                    "type": "string",
                    "enum": ["setup", "development", "climax", "resolution", "complete"],
                },
                "emotional_engagement": {
                    "type": "string",
                    "enum": ["high", "medium", "low", "overwhelmed"],
                },
                "next_question_type": {
                    "type": "string",
                    "enum": ["detail", "emotion", "context", "transition", "wrap_up"],
                },
    ...
    async def handle_request(self, params: FunctionCallParams):
        ...
        self.memory.conversation_state = params.arguments.get(
            "story_phase", self.memory.conversation_state
        )
